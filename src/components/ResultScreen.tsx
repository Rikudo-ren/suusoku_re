      ...
      <div className="relative z-10 flex h-full w-full items-center justify-center px-4 py-6">
        {stage >= 1 && (
          <div className="wipe-in clip-panel relative w-full max-w-5xl border border-cyan-400/30 bg-black/60 px-5 py-6 backdrop-blur-md md:px-12 md:py-8">
            ...
            {stage >= 5 && (
              <div className="rise-fade mt-6 space-y-4">
                {!submitted ? (
                  <div className="flex flex-col items-center gap-2">
                    <div className="font-mono2 text-[10px] tracking-[0.3em] text-cyan-300/60">
                      ランキングに登録
                    </div>
                    <div className="flex w-full max-w-sm flex-col gap-2 sm:flex-row">
                      <input
                        ...
                        className="clip-chip flex-1 border border-cyan-400/30 bg-black/50 px-4 py-2.5 font-ui text-sm text-white outline-none placeholder:text-white/30 focus:border-cyan-300"
                        disabled={submitting}
                      />
                      <button
                        disabled={submitting || stats.score <= 0}
                        onClick={async () => {
                          if (submitting || stats.score <= 0) return;
                          setSubmitting(true);
                          setSubmitError(null);
                          try {
                            await submitScore(
                        ...
                        className="clip-btn border border-cyan-400/50 bg-cyan-500/20 px-6 py-2.5 font-display text-sm font-black tracking-[0.2em] text-cyan-200 transition-colors hover:bg-cyan-500/30 disabled:opacity-40"
                      >
                        {submitting ? "送信中..." : "送信"}
                      </button>
                    </div>
                    {submitError && (
                      <div className="font-ui text-xs text-red-400">{submitError}</div>
                    )}
                  </div>
                ) : (
                  <div className="text-center font-mono2 text-sm tracking-widest text-cyan-300">
                    ✓ スコアを送信しました
                  </div>
                )}
                ...
                <div className="flex flex-col items-center gap-3 md:flex-row md:justify-center md:gap-4">
                  <button
                    ...
                    className="clip-btn group relative w-full overflow-hidden px-8 py-3 md:w-auto"
                    style={{
                  ...
                  <button
                    ...
                    className="clip-btn w-full border border-yellow-400/40 bg-yellow-500/10 px-8 py-3 font-display text-lg font-black tracking-[0.3em] text-yellow-200 transition-colors hover:bg-yellow-500/20 md:w-auto"
                  >
                    RANKING
                  </button>
                  <button
                    ...
                    className="clip-btn w-full border border-white/25 bg-white/5 px-8 py-3 font-display text-lg font-black tracking-[0.3em] text-white/70 transition-colors hover:bg-white/10 hover:text-white md:w-auto"
                  >
                    TITLE
                  </button>
                </div>
